...
function getLeagueMeta(leagueStr = '', home = '', away = '') {
  ...
  // 2. Substring matching in team dictionary (longer keys first for precision)
  const sortedTeams = Object.keys(TEAM_LEAGUES).sort((x, y) => y.length - x.length);
  for (const teamKey of sortedTeams) {
    const lName = TEAM_LEAGUES[teamKey];
    if (h && (h === teamKey || (teamKey.length >= 4 && h.includes(teamKey)) || (h.length >= 4 && teamKey.includes(h)))) {
      if (LEAGUE_META_MAP[lName]) return LEAGUE_META_MAP[lName];
    }
    if (a && (a === teamKey || (teamKey.length >= 4 && a.includes(teamKey)) || (a.length >= 4 && teamKey.includes(a)))) {
      if (LEAGUE_META_MAP[lName]) return LEAGUE_META_MAP[lName];
    }
  }
  ...
  return { key: 'Other', name: leagueStr || 'vFootball League', icon: '⚽', color: '#00E5FF' };
}

function getLiveMatchProgress(matchTime, now = new Date(), isInPlay = true) {
  // Convert current time to WAT (UTC+1) which matches the SportyBet virtual schedule
  ...
  if (matchTime && matchTime !== '--:--') {
    ...
    if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
      matchSec = parts[0] * 3600 + parts[1] * 60;
    }
  }

  let elapsedSec = 0;
  if (matchSec !== null) {
    let diffSec = (nowWatSec - matchSec + 86400) % 86400;
    if (diffSec > 43200) diffSec -= 86400; // Negative if scheduled in future

    if (diffSec < 0 && !isInPlay) {
      const secUntil = Math.abs(diffSec);
      const minsUntil = Math.ceil(secUntil / 60);
      return { 
        minuteText: secUntil <= 60 ? `Starts in ${secUntil}s` : `Starts in ${minsUntil}m`, 
        isLive: false 
      };
    }
    ...
    if (diffSec >= 0 && diffSec <= 150) {
      ...
      elapsedSec = Math.floor(now.getTime() / 1000) % 150;
    }
  } else {
    elapsedSec = Math.floor(now.getTime() / 1000) % 150;
  }

  // Active match simulation stages (150s window)
  if (elapsedSec <= 45) {
  ...
  if (elapsedSec > 60 && elapsedSec <= 135) {
    const min = Math.max(46, Math.min(89, Math.floor(45 + (elapsedSec - 60) * 0.6) + 1));
    return { minuteText: `${min}'`, isLive: true };
  }

  // Smooth final minute (never frozen at 90' FT)
...
export default function UnifiedMatchCenter() {
  ...
  // League Filtering Logic (Authoritative Team-First match)
  const filterByLeague = useCallback((m) => {
    if (activeLeague === 'ALL') return true;
    const homeName = m.home_team || m.homeTeam || m.home || '';
    const awayName = m.away_team || m.awayTeam || m.away || '';
    const meta = getLeagueMeta(m.league, homeName, awayName);
    const target = activeLeague.replace(' - Virtual', '').replace(' League', '').toLowerCase();
    const metaKey = (meta.key || meta.name || '').replace(' League', '').toLowerCase();
    return metaKey === target || meta.name.toLowerCase().includes(target);
  }, [activeLeague]);
              ...
              {displayedInPlay.length === 0 ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(380px, 1fr))', gap: '16px' }}>
                  {displayedInPlay.map((m, idx) => {
                    ...
                    const minuteDisplay = progress.minuteText;
